...
static inline uint8_t __get_uart_nvic_interrupt_enable_status(UartDriver_t* uart){
    return (uint8_t)(GET_BIT(*(uart->p_NVIC_ENABLE_REG), uart->nvic_enable_offset) & 0b1UL);
}

static inline uint8_t __get_uart_tx_interrupt_enable_status(UartDriver_t* uart){
    return (uint8_t)(GET_BIT((uart)->p_UART_BASE->CONTROL_REG_1, 7) & 0b1UL);
}

static inline uint8_t __get_uart_rx_interrupt_enable_status(UartDriver_t* uart){
