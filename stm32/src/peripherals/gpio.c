...
void gpio_configure_alt_function(gpio_t pin, AFKey af, PinSpeed speed, PinPullMode pull) {
    ...
    if (pin.offset < 8) {
        set_register(&((pin.base)->ALT_FUNC_CONFIG_BIT_0), pin.offset * 4, 4, (uint32_t)af);
    } else {
        set_register(&((pin.base)->ALT_FUNC_CONFIG_BIT_1), (pin.offset - 8) * 4, 4, (uint32_t)af);
    }
    // Set pin speed
    set_pin_speed(pin, speed);
